...
def write_transformation_to_csv_file(bag_file, target_frame, source_frame,
                ...
                if single_tf.header.frame_id == source_frame:
                    ...
                    if tf_counter > 3:
                        ...
                        tf_interval_deviation_percent = abs(
                            tf_interval_estimated - tf_interval_measured
                        ) / tf_interval_estimated * 100.
                        if (tf_interval_deviation_percent > 50.0):
                            seconds_from_start_time = (
                                single_tf.header.stamp.to_sec() -
                                start_time_tf_message.to_sec())
                            print(
                                "There might have been a drop in the tf after {:.3f}s.".
                                format(seconds_from_start_time))
                            print(
                                "\tThe interval deviated by {:.2f}%, interval: {:.6f}".
                                format(tf_interval_deviation_percent,
                                       tf_interval_measured))
                            print("\tCurrent frequency estimate: {:.2f}Hz".
                                  format(tf_frequency_estimated))
    ...
    if tf_counter > 3:
        ...
        print("Final estimate of tf topic frequency: ",
              "{0:.2f}".format(tf_frequency_estimated), "Hz")
