  ...
  """
    ...
    """
        if isinstance(other, Quaternion):
            x = (self.w * other.x + self.x * other.w + self.y * other.z -
                 self.z * other.y)
            y = (self.w * other.y - self.x * other.z + self.y * other.w +
                 self.z * other.x)
            z = (self.w * other.z + self.x * other.y - self.y * other.x +
                 self.z * other.w)
            w = (self.w * other.w - self.x * other.x - self.y * other.y -
                 self.z * other.z)
    ...
    def from_rotation_matrix(cls, rotation_matrix):
        # TODO(ff): Fix the case where w is <= 0.
        w = np.sqrt(1.0 + rotation_matrix[0, 0] + rotation_matrix[1, 1] +
    ...
    def rotate_vector(self, vector):
        ...
        vector_rotated[0] = (
            (1 - 2 * self.y**2 - 2 * self.z**2) * vector[0] + 2 *
            (self.x * self.y + self.w * self.z) * vector[1] + 2 *
            (self.x * self.z - self.w * self.y) * vector[2])
        vector_rotated[1] = (
            2 * (self.x * self.y - self.w * self.z) * vector[0] +
            (1 - 2 * self.x**2 - 2 * self.z**2) * vector[1] + 2 *
            (self.y * self.z + self.w * self.x) * vector[2])
        vector_rotated[2] = (
            2 * (self.x * self.z + self.w * self.y) * vector[0] + 2 *
            (self.y * self.z - self.w * self.x) * vector[1] +
...
def quaternion_slerp(q_1, q_2, fraction):
  ...
  """
    ...
    assert np.isclose(
        q_1.norm(), 1.0,
        atol=1.e-8), "Slerp should only be used with unit quaternions."
    assert np.isclose(
        q_2.norm(), 1.0,
        atol=1.e-8), "Slerp should only be used with unit quaternions."
